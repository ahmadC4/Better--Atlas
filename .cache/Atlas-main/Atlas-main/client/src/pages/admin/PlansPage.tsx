                ...
                <CardContent className="space-y-5">
                  ...
                  <div className="space-y-3">
                    ...
                    {FEATURE_TOGGLES.map((feature: FeatureToggle) => {
                      ...
                      const meetsModelRequirement = !feature.requiresModelIds || feature.requiresModelIds.some((id) => allowedModels.has(id));
                      const meetsCapabilityRequirement = !feature.requiresCapabilities || allowedModelDetails.some((model) => feature.requiresCapabilities?.every((cap) => model.capabilities.includes(cap)));
                      const supportsFeature = meetsModelRequirement && meetsCapabilityRequirement;
                      const missingModels = feature.requiresModelIds
                        ?.filter((id) => !allowedModels.has(id))
                        .map((id) => chatModels.find((model) => model.id === id)?.name ?? id);
                      const requirementMessage = supportsFeature
                        ? undefined
                        : missingModels && missingModels.length > 0
                          ? `Requires enabling ${missingModels.join(', ')}.`
                          : feature.requiresCapabilities && feature.requiresCapabilities.length > 0
                            ? `Requires a model with ${feature.requiresCapabilities.map((cap) => capabilityLabels[cap] ?? cap).join(' + ')}.`
                            : undefined;
                      ...
                      return (
                        <div
                          ...
                          data-testid={`toggle-plan-${tier}-feature-${feature.id}`}
                        >
                          <div className="space-y-1">
                            <p className="text-sm font-medium leading-none">{feature.label}</p>
                            <p className="text-xs text-muted-foreground">{feature.description}</p>
                            {requirementMessage && (
                              <p className="text-xs text-amber-600 dark:text-amber-400">{requirementMessage}</p>
                            )}
                          </div>
                          <Switch
                            checked={isEnabled}
