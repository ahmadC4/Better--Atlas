...
export function Chat() {
  ...
  const previousPlanRef = useRef<UserPlan>('free');
  ...
  const enabledFeatures = userLimits?.features ?? [];
  ...
  useEffect(() => {
    const defaultModelId = userLimits?.allowedModels?.[0] || 'compound';
    ...
    previousPlanRef.current = plan;
  }, [plan, availableModels, selectedModel, userLimits?.allowedModels]);
