...
const ChatMessages = forwardRef<ChatMessagesHandle, ChatMessagesProps>(({ 
  ...
  const messageContainerBaseClasses =
    'relative flex-1 min-w-0 w-full max-w-[95vw] sm:max-w-[85vw] md:max-w-3xl rounded-2xl px-4 py-3 sm:px-5 sm:py-4 shadow-sm break-words overflow-hidden';
        ...
        <div className="mx-auto w-full max-w-4xl space-y-6">
          ...
          {messages.map((message) => {
            ...
            const isVoiceMessage =
              !isUser && (Boolean(metadata?.voiceMode) || (metadataVoiceClips?.length ?? 0) > 0);
