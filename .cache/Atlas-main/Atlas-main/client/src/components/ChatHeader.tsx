import { Menu, ChevronDown, Zap, Activity, MessageSquarePlus } from 'lucide-react';
...
export function ChatHeader({
  ...
  return (
    <header
      className={cn(
        'grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 sm:gap-4 p-3 sm:p-6 border-b border-border bg-background shadow-sm',
        className,
      )}
    >
      <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0">
        ...
        <button
          ...
          className="flex items-center gap-1.5 sm:gap-2 rounded-full px-2 py-1 transition-colors hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
        >
          <Zap className="h-4 w-4 sm:h-5 sm:w-5 text-primary" aria-hidden />
      ...
      <div className="flex min-w-0 justify-center">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="outline"
              className="mx-auto flex w-full min-w-0 max-w-[min(360px,80vw)] items-center justify-between gap-2 rounded-xl px-3 py-2 text-xs shadow-sm sm:mx-0 sm:max-w-xs sm:text-sm"
          ...
          <DropdownMenuContent align="center" className="w-72 rounded-xl border shadow-lg">
